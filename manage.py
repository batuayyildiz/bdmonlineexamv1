...
def create_database():
    ...
    with db.get_cursor() as cursor:
        ...
        cursor.execute("CREATE TABLE Sinav(sinav_id serial PRIMARY KEY,ogretmen_id INTEGER NOT NULL,sinav_adi VARCHAR(50) NOT NULL,sinav_baslama_zamani TIMESTAMP NOT NULL,sinav_bitis_tarihi TIMESTAMP NOT NULL,FOREIGN KEY (ogretmen_id) REFERENCES Kullanici(kullanici_id));")
        ...
        cursor.execute("CREATE TABLE Ogrenci_Sinav(ogrenci_id INTEGER,sinav_id INTEGER,ogrenci_sinav_bitisi TIMESTAMP NOT NULL,dogru_sayi INTEGER NOT NULL ,yanlis_cevap INTEGER NOT NULL ,puan INTEGER NOT NULL,PRIMARY KEY(ogrenci_id,sinav_id),FOREIGN KEY(ogrenci_id) REFERENCES Kullanici(kullanici_id),FOREIGN KEY(sinav_id) REFERENCES Sinav(sinav_id));")
    db.commit()
